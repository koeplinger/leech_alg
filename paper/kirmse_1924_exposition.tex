...
The conclusion is this.  Kirmse built, essentially in full, the arithmetic
theory of the integral octonions: the units and the multiplication table,
...
arithmetic is exact and is reproducible from the script
\texttt{python\_project/src/verify\_kirmse\_1924.py} in the
repository~\cite{repo}; the run reported here was performed under prompt~100
of the research record.  Item~(b)'s witness~\eqref{eq:witness} was also
checked by hand.  The count of $30$ doubly-even self-dual binary codes of
length~$8$ is classical; item~(c) re-derives it and the figure ``seven'' is
the output of the closure test, not a quotation.
\end{tracing}
...
Two matters are deliberately left open here.  First, who detected the error
and in what form the count was corrected: that is the subject of Coxeter's
1946 paper, and this exposition will be revised once that paper has been read
from the source in the same manner as Kirmse's.  Second, whether the other
seven members of Kirmse's claimed eight coincide with the seven genuine
maximal orders: Kirmse gives only the schematic form~\eqref{eq:eight}, never
an explicit list, so this cannot be settled from his text and is not claimed
here.
...
Verification script: \texttt{python\_project/src/verify\_kirmse\_1924.py};
research record: prompt~100.
