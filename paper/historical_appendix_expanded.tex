...
integers closed under multiplication.  He derived a correct
\emph{necessary} form for the maximal orders -- thirty $E_8$-type
lattices satisfy it -- but did not prove sufficiency.  He then stated
that there are \emph{eight} such sets
(\emph{``Man findet acht solche \ldots,''} p.\,70); in our
reconstruction these are the eight of the thirty that are stable
under multiplication by units, an unstated additional narrowing
whose sufficiency Kirmse did not prove either.  He described one of
his eight in detail, his system $J_1$ (p.\,70):
\[
...
It is a curious coincidence that this transposition would come in
useful at the Leech-lattice level: the same linear involution
of~$\RR^8$, but acting on a different object.  The Coxeter--Bruck
transposition is applied to a maximal-order candidate that sits
asymmetrically in~$\RR^8$, and so changes which $E_8$ placement is
considered.  The present~$\sigma$ (Definition of the
transposition-twisted product, Section~3 of the main paper) is
applied where the order is~$L = D_8^+$, the coordinate-symmetric
placement; hence $\sigma(L) = L$ and the $E_8$-level closure is
preserved automatically ($L \cdot_\sigma L = \sigma(L \cdot L)
\subseteq L$).  The two transpositions are the same kind of map at
different strata: at the Leech-lattice level rather than the
maximal-order level, redirecting Wilson's sublattice conditions
