...
\address{Anthropic}
\date{16 April 2026; revised 23 May 2026 (v4 companion)}

\begin{document}
\maketitle

\begin{abstract}
This companion note was written by the AI agent (drafted by
Claude Opus~4.6 in max-thinking mode, revised 23 May 2026 by
Claude Opus~4.7, Anthropic) at the direction of Jens K\"oplinger,
the author of the accompanying paper ``An order on the Leech lattice
from transposition-twisted octonions.''  It is a \emph{training
document}, assembled from the overall corpus of the project
repository
\href{https://bitbucket.org/jenskoeplinger/leech_alg}%
     {\texttt{bitbucket.org/jenskoeplinger/leech\_alg}},
intended as a learning tool for the principal investigator and for
other readers who approach this construction with the kind of
questions -- and, candidly, misconceptions -- that arose throughout
the investigation.  Authorship and intellectual leadership of the
main paper and the research programme belong to K\"oplinger, with AI
collaboration as acknowledged there; the present companion is solely
the AI agent's rendering of that work in expository form and makes no
claim to new mathematics.

The note walks through the construction step by step, pairing every
abstract definition with a concrete hand-checkable example, first at
the~$E_8$ level (where Coxeter's correction of Kirmse is the
historical example of the same kind of operation) and then at the
Leech level (where the $\sigma$-twist applies that same kind of map
to a different object).  Readers who are
comfortable with the main paper will find nothing new here; readers
who want to see every piece verified by hand will find the explicit
numerics they may have missed.
\end{abstract}
...
% ===================================================================
\section{The Kirmse--Coxeter example}\label{sec:kirmse}
% ===================================================================

\subsection*{What Kirmse did, and where it went wrong}

Kirmse (1924) fixed one octonion multiplication -- call it
$\cdot_K$ -- given by the seven Fano triples
\[
  (1,2,3),\ (1,5,4),\ (1,6,7),\ (2,4,7),\ (2,6,5),\ (3,4,6),\ (3,5,7)
\]
(read cyclically, as in \eqref{eq:fano}; this labelling differs
from~\eqref{eq:fano}).  He then looked for the \emph{maximal orders}:
the largest lattices in $(\RR^8, \cdot_K)$ closed under multiplication.
His table $\cdot_K$ is itself a perfectly correct octonion algebra --
that is \emph{not} where the error lies.  He listed eight candidate
maximal orders and described one of them, $J_1$, in detail.

$J_1$ is the lattice spanned over~$\ZZ$ by the eight units
$1, i_1, \ldots, i_7$ together with the four half-integer octonions
\[
  a_1 = \tfrac12(1{+}i_1{+}i_2{+}i_3), \qquad
  a_2 = \tfrac12(i_4{+}i_5{+}i_6{+}i_7),
\]
\[
  a_3 = \tfrac12(1{+}i_1{+}i_4{+}i_5), \qquad
  a_4 = \tfrac12(1{+}i_2{+}i_4{+}i_7).
\]
Kirmse's error is that $J_1$ is \emph{not closed} under~$\cdot_K$.
The witness, found by Coxeter (1946) and reproduced in exact
arithmetic in \texttt{verify\_coxeter\_1946.py}, is
\[
  a_1 \cdot_K a_3
  = \tfrac12(1{+}i_1{+}i_2{+}i_3)\cdot_K\tfrac12(1{+}i_1{+}i_4{+}i_5)
  = \tfrac12(i_1{+}i_2{+}i_4{+}i_7),
\]
and $\tfrac12(i_1{+}i_2{+}i_4{+}i_7) \notin J_1$.  So $J_1$ is not an
order at all.  There are in fact \emph{seven} maximal orders
for~$\cdot_K$, not eight; $J_1$ is the spurious entry on Kirmse's list.
Coxeter identified the mistake; R.~H.~Bruck supplied the remedy.

\subsection*{The remedy: a transposition of two coordinate axes}

Bruck's correction, as Coxeter reports it, is to ``transpose two of
the $i$'s.''  Concretely: pick a transposition of two imaginary
basis axes -- a linear involution of~$\RR^8$ that swaps
$e_a \leftrightarrow e_b$ and fixes the other six.  \emph{Any} of
the $\binom72 = 21$ such transpositions repairs~$J_1$ (Coxeter; the 21
fall into 7 triples of equal effect, giving the 7 maximal orders --
verified).

We write~$\sigma$ for such a transposition, the \emph{same} symbol the
main paper uses -- because it is the same kind of map, and disguising
that with two symbols would only obscure the point of this note.  What
differs between the Kirmse--Coxeter use and the main paper's use is
never the map: it is the \emph{algebra and lattice the map is applied
to}, and we keep that explicit at every step.  For definiteness take
$\sigma = (1\;2)$, the swap $e_1 \leftrightarrow e_2$ -- which is at
once one of the 21 transpositions that repair Kirmse's~$J_1$ and the
transposition the main paper twists by.

The repair has two equivalent descriptions, and it is worth being
completely explicit about \emph{which object each one moves.}

\smallskip
\noindent\emph{Framing A -- $\sigma$ acts on the lattice (on the vector
space).}  Keep the product~$\cdot_K$.  Replace the lattice~$J_1$ by its
image $J := \sigma(J_1)$.  Then $(J, \cdot_K)$ \emph{is} a maximal order:
$J$ is closed under~$\cdot_K$.  The algebra is untouched; what changed
is the subset of~$\RR^8$ being tested.

\smallskip
\noindent\emph{Framing B -- $\sigma$ acts on the product (on the
algebra).}  Keep the lattice~$J_1$.  Replace~$\cdot_K$ by its
$\sigma$-conjugate
\[
  a \cdot_K^{\,\sigma} b \;:=\; \sigma\bigl(\sigma(a) \cdot_K \sigma(b)\bigr).
\]
Then $(J_1, \cdot_K^{\,\sigma})$ is a maximal order: $J_1$ is closed
under~$\cdot_K^{\,\sigma}$.  The lattice is untouched; what changed is the
multiplication.

\smallskip
These are not two corrections but one.  $\sigma$ is a single linear map,
and $\sigma\colon (\RR^8, \cdot_K^{\,\sigma}) \to (\RR^8, \cdot_K)$ is an
algebra \emph{isomorphism}, so it carries closed sublattices to closed
sublattices: $J_1$ is closed under~$\cdot_K^{\,\sigma}$ \emph{iff}
$\sigma(J_1)$ is closed under~$\cdot_K$.  Because~$\sigma$ is orthogonal,
$\cdot_K^{\,\sigma}$ is again a \emph{composition} algebra -- again an
octonion algebra; the ``corrected table'' is just another octonion
table, isomorphic to Kirmse's via~$\sigma$.

\begin{example}[What the Kirmse--Coxeter twist modifies]
Take $\sigma = (1\;2)$.  In framing~B it rewrites exactly those basis
products that involve a swapped axis, and leaves the rest alone
(verified in \texttt{verify\_coxeter\_1946.py}):
\[
  \begin{array}{lcl}
    i_1 \cdot_K i_2 = +i_3 & \quad\leadsto\quad
      & i_1 \cdot_K^{\,\sigma} i_2 = -i_3, \\[2pt]
    i_1 \cdot_K i_5 = +i_4 & \leadsto
      & i_1 \cdot_K^{\,\sigma} i_5 = -i_6, \\[2pt]
    i_6 \cdot_K i_7 = +i_1 & \leadsto
      & i_6 \cdot_K^{\,\sigma} i_7 = +i_2, \\[2pt]
    i_3 \cdot_K i_4 = +i_6 & \leadsto
      & i_3 \cdot_K^{\,\sigma} i_4 = +i_6 \quad (\text{unchanged}).
  \end{array}
\]
And on the very pair that broke closure -- the same two
elements~$a_1, a_3$ of the same lattice~$J_1$:
\[
  a_1 \cdot_K a_3 = \tfrac12(i_1{+}i_2{+}i_4{+}i_7) \notin J_1,
  \qquad
  a_1 \cdot_K^{\,\sigma} a_3 = \tfrac12(i_1{+}i_3{+}i_4{+}i_7) \in J_1.
\]
Twisting the product (framing~B) restored closure without moving the
lattice.
\end{example}
...
From here on we return to the multiplication~$\cdot$ of
\eqref{eq:fano} -- one of Coxeter's seven corrected octonion tables
-- and to the lattice~$L$, which closes under it.

Whether a transposition twist changes anything depends entirely on the
lattice it is asked about.  For the lattice~$L$ itself the twist is
invisible: $L$ is symmetric under every coordinate permutation, so
transposing two axes produces an algebra under which $L$ also closes
(this is made precise in Section~10).  Kirmse's~$J_1$, by contrast, is
\emph{not} symmetric under such a swap -- which is exactly why the
twist moved it.  A second lattice of that non-symmetric kind, the one
that drives the rest of this note, appears naturally in Wilson's Leech
construction: the sublattice
\[
...
% ===================================================================
\section{The $\sigma$-twist, abstractly}\label{sec:swap}
% ===================================================================
...
octonion conjugate of the negative of the earlier~$s$, again of
squared norm~$2$.  Note $\bar s \notin L$ under the lattice
membership criterion of Section~\ref{sec:e8}: its coordinates are
half-integer but its coordinate sum is~$-4$, which falls in the
excluded even branch of the half-integer case.  Define two
sublattices of~$L$:
\[
  Ls = \{\ell \cdot s : \ell \in L\}, \qquad
  L\bar{s} = \{\ell \cdot \bar{s} : \ell \in L\},
\]
each of index~$16$ in~$L$.  These sets are sublattices of~$L$
despite $\bar s \notin L$: for every $\ell \in L$ the product
$\ell \cdot \bar s$ nonetheless lies in~$L$ (verified directly on
the 240 roots and extended by linearity).  Wilson's Leech lattice is then
...
A natural concern.  Coxeter's correction of Kirmse and the
present~$\sigma$ are both transpositions of two imaginary coordinate
axes -- the same kind of map, a linear involution of~$\RR^8$
(Section~\ref{sec:kirmse}); indeed $\sigma = (1\;2)$ is one of the
$21$ transpositions that repair Kirmse's~$J_1$.  Could a poor choice
of~$\sigma$, then, reverse Coxeter's correction and return the $E_8$
lattice to a non-closing regime?

It cannot, and the reason is one short computation.

\begin{proposition}
For every coordinate transposition~$\sigma$, the lattice $L = D_8^+$
remains closed under the twisted product:
$L \cdot_\sigma L \subseteq L$.
\end{proposition}

\begin{proof}
$L = D_8^+$ is invariant under every coordinate permutation, so
$\sigma(L) = L$.  The main paper establishes $L \cdot L \subseteq L$.
Hence
\[
  L \cdot_\sigma L
  = \sigma\bigl(\sigma(L) \cdot \sigma(L)\bigr)
  = \sigma(L \cdot L)
  \subseteq \sigma(L) = L. \qedhere
\]
\end{proof}

So no $\sigma$-twist can disturb the $E_8$-level closure.  This is
exactly Wilson's condition~(1): for $u, v \in \LL \subseteq L^3$, every
block of $u \star_\sigma v$ is a sum of twisted products
$x \cdot_\sigma x'$ with octonion factors $x, x' \in L$, and the
proposition returns each block to~$L$.  Condition~(1) holding
\emph{is} Coxeter's closure carried through the twist; the
construction depends on the correction, it does not undo it.

The twist does act non-trivially -- but only on the lattices
$\sigma$ does \emph{not} fix.  $L$ is fixed; the sublattices $Ls$ and
$L\bar s$ are not, and that is where Wilson's conditions~(2) and~(3)
are redirected.  There is no tension between ``$\sigma$ fixes~$L$'' and
``$\sigma$ moves Kirmse's~$J_1$'': $L$ and~$J_1$ are both copies
of~$E_8$, but \emph{placed} differently in~$\RR^8$ -- $L = D_8^+$ is
the coordinate-symmetric placement, $J_1$ is not (it is fixed by none
of the $21$ transpositions).  Whether a transposition fixes a lattice
is a property of the lattice's placement, not of the map.  Taking the
order to be the symmetric placement~$D_8^+$ is exactly what makes the
twist free at the $E_8$ level; Coxeter's transposition, asked instead
about the non-symmetric~$J_1$, necessarily moves it.
