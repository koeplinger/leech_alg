...
- **Appendix Tables A.1, A.2, A.3 — all 192 coefficient rows.** Every one of the 64 products $L_i\cdot L_j$, $L_i\cdot N_j$, $M_i\cdot M_j$ was recomputed and expressed in the target basis. **All 192 rows are integer, and all 192 match the printed tables entry-for-entry.** This is the logical content of Lemmas 4.3, 4.4, 4.5, and hence of Theorem 1.1; it checks out completely.
- **Remark 3.5 (table diffs):** for $\sigma=(1\;2)$, exactly **30** of 64 multiplication-table entries change — **6** sign-only, **16** target-only, **8** both. Matches the manuscript.
- **Remark 4.6 (the witness):** $v=(-1,0,1,0,0,1,0,1)$ lies in $\sigma(Ls)$ and is **not** an integer combination of a $\mathbb{Z}$-basis of $Ls$ (it requires half-integer coefficients). Confirmed. Independently, $Ls$ is genuinely **not** closed under the standard octonion product (40 of 64 basis-pair products leave $Ls$) — the obstruction the twist resolves is real.
...
- Propositions 4.7/4.9/4.11 each invoke exactly the lemma they need (4.3, 4.4, 4.5 respectively), and the input hypotheses they consume (Wilson's conditions on $u$ and $v$) are all available because $u,v\in\Lambda$. The use of $u$'s condition (1) together with $v$'s condition (2) in Proposition 4.9 is a legitimate one-sided grouping of a bilinear expression — not an error.
...
### 2.2 Redundancy in Definition 2.4
